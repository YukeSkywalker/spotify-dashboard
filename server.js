...
// Questa funzione viene usata internamente per garantire che il token sia sempre valido
...
app.get('/login', (req, res) => {
    ...
    const scope = 'user-read-currently-playing user-top-read user-read-playback-state user-modify-playback-state streaming user-read-email user-read-private playlist-read-private';
...
// --- API: STATO AUTH ---
app.get('/api/auth-status', (req, res) => res.json({ loggedIn: !!tokenStore.access_token }));
app.get('/api/token', async (req, res) => {
    try { res.json({ token: await getAccessToken() }); } catch (e) { res.status(401).json({ error: 'Auth fail' }); }
});

// --- API: DATI UTENTE E TOP ---
...
app.get('/api/top-tracks', async (req, res) => {
    try {
        const token = await getAccessToken();
        const { range = 'short_term', limit = 10 } = req.query;
...
app.post('/api/player/volume', async (req, res) => {
    try {
        const token = await getAccessToken();
        await axios.put(`https://api.spotify.com/v1/me/player/volume?volume_percent=${req.query.volume}`, {}, { headers: { Authorization: `Bearer ${token}` } });
        res.json({ ok: true });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/search', async (req, res) => {
    try {
        const token = await getAccessToken();
        const { data } = await axios.get(`https://api.spotify.com/v1/search?q=${encodeURIComponent(req.query.q)}&type=track&limit=15`, {
            headers: { Authorization: `Bearer ${token}` }
        });
        res.json(data);
    } catch (e) { res.status(500).json({ error: e.message }); }
...
app.post('/api/player/queue', async (req, res) => {
    try {
        const token = await getAccessToken();
        await axios.post(`https://api.spotify.com/v1/me/player/queue?uri=${encodeURIComponent(req.query.uri)}`, {}, {
            headers: { Authorization: `Bearer ${token}` }
        });
        res.json({ ok: true });
    } catch (e) { res.status(500).json({ error: e.message }); }
...
// --- AVVIO ---
app.listen(PORT || 3000, () => console.log(`Dashboard attiva su http://localhost:${PORT || 3000}`));
